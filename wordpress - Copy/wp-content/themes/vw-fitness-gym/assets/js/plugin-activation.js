jQuery(document).ready(function($) {
    'use strict';
    var this_obj = vw_fitness_gym_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.vw-fitness-gym-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message vw-fitness-gym-plugin-install');
                button.addClass('button-primary vw-fitness-gym-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.vw-fitness-gym-recommended-plugins .vw-fitness-gym-activation-note').css('display','block');
    ...
    $(document).on('click', '.vw-fitness-gym-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .vw-fitness-gym-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .vw-fitness-gym-recommended-plugins').css('display','none');
    });
